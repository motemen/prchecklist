...
func (u Usecase) RemoveCheck(ctx context.Context, clRef prchecklist.ChecklistRef, featNum int, user prchecklist.GitHubUser) (*prchecklist.Checklist, error) {
	...
	go func(ctx context.Context, cl *prchecklist.Checklist) {
		...
		if err := u.setRepositoryCompletedStatusAs(ctx, cl.Owner, cl.Repo, lastCommitID, "pending", cl.Stage); err != nil {
...
func (u Usecase) setRepositoryCompletedStatusAs(ctx context.Context, owner, repo, ref, state, stage string) error {
	return u.github.SetRepositoryStatusAs(ctx, owner, repo, ref, fmt.Sprintf("prchecklist/%s/completed", stage), state)
}
